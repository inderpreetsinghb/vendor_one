...
func init() {
	android.RegisterModuleType("one_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = oneExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = oneExpandVariables(ctx, dep_file)
	...
	cmd := oneExpandVariables(ctx, String(g.properties.Cmd))
