...
func init() {
	android.RegisterModuleType("caf_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = cafExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = cafExpandVariables(ctx, dep_file)
	...
	cmd := cafExpandVariables(ctx, String(g.properties.Cmd))
